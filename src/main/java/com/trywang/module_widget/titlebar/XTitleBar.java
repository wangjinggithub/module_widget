...
public class XTitleBar extends FrameLayout {
    ...
    public XTitleBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.TitleBar_TextLeft) {
                mTextLeft = a.getString(attr);

            } else if (attr == R.styleable.TitleBar_TextCenter) {
                mTextCenter = a.getString(attr);

            } else if (attr == R.styleable.TitleBar_TextRight) {
                mTextRight = a.getString(attr);

            } else if (attr == R.styleable.TitleBar_TextColorLeft) {
                mTextColorLeft = a.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.TitleBar_TextColorCenter) {
                mTextColorCenter = a.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.TitleBar_TextColorRight) {
                mTextColorRight = a.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.TitleBar_TextSizeLeft) {
                mTextSizeLeft = a.getDimensionPixelSize(attr, 12);

            } else if (attr == R.styleable.TitleBar_TextSizeCenter) {
                mTextSizeCenter = a.getDimensionPixelSize(attr, 18);

            } else if (attr == R.styleable.TitleBar_TextSizeRight) {
                mTextSizeRight = a.getDimensionPixelSize(attr, 12);

            } else if (attr == R.styleable.TitleBar_isShowImgLeft) {
                isShowImgLeft = a.getBoolean(attr, true);

            } else if (attr == R.styleable.TitleBar_isShowImgRight) {
                isShowImgRight = a.getBoolean(attr, true);

            } else if (attr == R.styleable.TitleBar_isShowLeft) {
                isShowLeft = a.getBoolean(attr, true);

            } else if (attr == R.styleable.TitleBar_isShowRight) {
                isShowRight = a.getBoolean(attr, false);

            } else if (attr == R.styleable.TitleBar_srcLeft) {
                mDrawLeft = a.getDrawable(attr);

            } else if (attr == R.styleable.TitleBar_srcRight) {
                mDrawRight = a.getDrawable(attr);

            } else if (attr == R.styleable.TitleBar_bg) {
                mDrawBg = a.getDrawable(attr);

            } else if (attr == R.styleable.TitleBar_onClickLeft) {
                ...
                if (handlerNameLeft != null) {
                    setmTitleBarLeftListener(new TitleBarLeftListener() {
                        ...
                        public void onClickTitleLeftListener(View v) {
                            new DeclaredOnClickListener(mLayLeft, handlerNameLeft).onClick(v);
                        }
                    });
                }

            } else if (attr == R.styleable.TitleBar_onClickCenter) {
                if (context.isRestricted()) {
                ...
                if (handlerNameCenter != null) {
                    setmTitleBarCenterListener(new TitleBarCenterListener() {
                        ...
                        public void onClickTitleCenterListener(View v) {
                            new DeclaredOnClickListener(mLayCenter, handlerNameCenter).onClick(v);
                        }
                    });
                }

            } else if (attr == R.styleable.TitleBar_onClickRight) {
                ...
                if (handlerName != null) {
                    setmTitleBarRightListener(new TitleBarRightListener() {
                        ...
                        public void onClickTitleRightListener(View v) {
                            ...
                            new DeclaredOnClickListener(mLayRight, handlerName).onClick(v);
                        }
                    });
                }
    ...
    public void init(Context c){
        ...
        mTxtLeft = findViewById(R.id.tv_title_left);
        mTxtCenter = findViewById(R.id.tv_title_center);
        mTxtRight = findViewById(R.id.tv_title_right);
        mLayLeft = findViewById(R.id.fl_title_left);
        mLayCenter = findViewById(R.id.fl_title_center);
        mLayRight = findViewById(R.id.fl_title_right);
        mImgBg = findViewById(R.id.iv_title_bg);
    ...
    private void onClickLeftDefault(){
